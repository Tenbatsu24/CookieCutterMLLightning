...
class GeneralizedPRIMEModule(torch.nn.Module):
    def __init__(self, aug_module, mixture_width=3, mixture_depth=-1, max_depth=3):
        """
    ...
    def aug(self, img):
        ...
        dirichlet = Dirichlet(
            concentration=torch.tensor([1.0] * self.mixture_width, device=img.device)
        )
        beta = Beta(
            concentration1=torch.ones(1, device=img.device, dtype=torch.float32),
        ...
        depth_mask = torch.zeros(
            img_repeat.shape[0], self.max_depth, 1, 1, 1, device=img_repeat.device
        )
        trans_mask = torch.zeros(
            img_repeat.shape[0], self.aug_module.num_transforms, 1, 1, 1, device=img_repeat.device
        )
        ...
        for d in range(self.depth):
            ...
            image_aug.data = (
                depth_mask[:, d] * self.aug_module(image_aug, trans_mask)
        ...
        mixed = (1.0 - m) * img + m * mix

        return mixed.reshape(init_shape)

    def __repr__(self):
        mixture_width, mixture_depth, max_depth, aug_module = (
            self.mixture_width,
            self.mixture_depth,
            self.max_depth,
            self.aug_module,
        )
        return f"GeneralizedPRIMEModule({mixture_width=}, {mixture_depth=}, {max_depth=}, {aug_module=})"
