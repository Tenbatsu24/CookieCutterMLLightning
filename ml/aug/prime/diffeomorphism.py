    ...
    *the allowed T interval is defined such as:
    ...
    def __init__(self, sT, rT, scut, rcut, cut_min, cut_max, alpha, stochastic=False):
        ...
        self.betaT = torch.distributions.beta.Beta(
            sT - sT / (rT + 1), sT / (rT + 1), validate_args=None
        )
        self.betacut = torch.distributions.beta.Beta(
            scut - scut / (rcut + 1), scut / (rcut + 1), validate_args=None
        )
    ...
    def _sample_params(self):
        self.cutmax = torch.randint(
            low=self.cutmin + 1, high=self.cutmax_max + 1, size=(1,)
        ).item()
    ...
    def __repr__(self):
        return (
            ...
            + f"(sT={self.sT}, rT={self.rT}, scut={self.scut}, rcut={self.rcut}, cutmin={self.cutmin}, cutmax={self.cutmax})"
        )
...
def remap(a, dx, dy, interp):
    ...
    """
    n, m = a.shape[-2:]
    assert dx.shape == (n, m) and dy.shape == (
        n,
        m,
    ), "Image(s) and displacement fields shapes should match."

    y, x = torch.meshgrid(
        torch.arange(n, dtype=dx.dtype, device=a.device),
