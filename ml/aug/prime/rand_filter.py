...
class RandomFilter(torch.nn.Module):
    def __init__(self, kernel_size, sigma, stochastic=False, sigma_min=0.0):
        super().__init__()
    ...
    def forward(self, img):
        ...
        conv_weight = rearrange(
            self.sigma
            * torch.randn((batch_size, self.kernel_size, self.kernel_size), device=img.device)
            + delta,
            "b h w -> b (h w)",
        )
        ...
        filtered_img = torch.nn.functional.conv2d(
            img, conv_weight, padding="same", groups=batch_size, stride=1, dilation=1
        )
        ...
        filtered_img = torch.clamp(filtered_img, 0.0, 1.0).reshape(init_shape)

        return filtered_img

    def _sample_params(self):
        self.kernel_size = int(
            self.kernels_size_candidates[torch.multinomial(self.kernels_size_candidates, 1)].item()
        )
